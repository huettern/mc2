...
// Copyright (c) 2017 Matthias Meier, adapted for STM32F429DISC1 board
//
...
// board STM32F429DISC1 (Green User LED on PG13, active high)
// Port numbers: 0=A, 1=B, 2=C, 3=D, 4=E, 5=F, 6=G, ...
#define BLINK_PORT_NUMBER               (6)
#define BLINK_PIN_NUMBER                (13)
#define BLINK2_PIN_NUMBER                (14)
...
#if (BLINK_ACTIVE_LOW)
    HAL_GPIO_WritePin(BLINK_GPIOx(BLINK_PORT_NUMBER),
        BLINK_PIN_MASK(BLINK2_PIN_NUMBER), GPIO_PIN_RESET);
#else
    HAL_GPIO_WritePin(BLINK_GPIOx(BLINK_PORT_NUMBER),
        BLINK_PIN_MASK(BLINK2_PIN_NUMBER), GPIO_PIN_SET);
#endif
}
